...
\cventry{2025.06--present}{Provenance \cvnote{(code plagiarism detection)}}
  {Personal project, Rust}
  {\cvlink{https://github.com/99JIK/Provenance}{github.com/99JIK/Provenance}}
  {Compares program structure rather than text, so renaming variables or rewriting
   comments does not hide a copy. Parses Python, Java, C, C++, JavaScript, TypeScript,
   and Go, with the parsers compiled into the binary so a grader installs nothing else.
   Starter code can be set as a template and discounted. Matches are reported as groups
   rather than pairwise, scored by similarity, containment, and density.
