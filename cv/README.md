...
**제출본은 커밋된 `cv-ko.pdf`와 `cv-en.pdf`다.** 로컬에서 빌드해 **소스와 같은 커밋에**
넣는다. 소스만 커밋하면 CI가 막는다. 컴파일은 되니까 예전에는 아무도 못 잡던 실수다.

빌드는 재현된다. 같은 소스면 언제 빌드하든 PDF 바이트가 같으므로, `git status`가
깨끗하다는 것이 곧 PDF가 소스와 맞다는 뜻이다. 확인하려면 그냥 다시 빌드해 보면 된다.
차이가 나면 소스를 고치고 빌드를 안 한 것이다.

CI는 결과물을 만들지 않는다. 컴파일 여부, PDF 최신성, 그리고 태그에서는 TODO 잔존
여부를 본다.
...
**로컬 빌드는 TODO를 막지 않는다.** `\TODO{}`가 남아 있어도 latexmk는 종료 코드 0으로
빨간 표식이 찍힌 PDF를 정상 생산한다. 제출 직전에 이걸 걸러 주는 건 태그 게이트뿐이라,
제출본은 반드시 태그를 붙여서 CI를 한 번 태울 것.
